...
const BlogFormDrawer = ({ isOpen, onClose, blogId, initialData, onSaved }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (blogId) {
        await api.put(`/admin/blogs/${blogId}`, formData);
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div
        ...
        className="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col overflow-y-auto border-l border-rule bg-paper shadow-[0_0_40px_rgba(0,0,0,0.18)]"
      >
        <header className="border-b border-rule px-6 py-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="file-index-sm">
                {blogId ? `AMEND · BK-BLG-${blogId.slice(-4)}` : 'FILE · NEW'}
              </p>
              <h2
                id="drawer-title"
                className="mt-0.5 text-title font-semibold text-ink"
              >
                {blogId ? 'Amend record' : 'File new record'}
              </h2>
            </div>
            <button
              type="button"
        ...
        <div className="px-6 py-5">
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label className="mb-1 block font-mono text-[12px] uppercase tracking-[0.08em] text-ink-muted" htmlFor="f-description">
                  Description
                </label>
                <textarea
                  id="f-description"
                  ...
                  required
                  className={fieldClass}
                  placeholder="Brief description for the index…"
                />
              </div>
              <div>
            ...
            <div className="mt-6 flex flex-row-reverse gap-3">
              <button type="submit" disabled={isSubmitting} className="btn btn-primary">
                {isSubmitting ? 'Saving…' : blogId ? 'Save amendments' : 'File record'}
              </button>
