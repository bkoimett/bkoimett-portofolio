...
|---|---|---|
| POST `/api/admin/login` | rate-limited (5/15m) | Vercel fn + Supabase Auth (Phase 5) |
| PUT `/api/admin/settings` | JWT | retained (Phase 5) |
| GET `/api/projects` | optional JWT (published-only public) | Vercel fn → `projects` |
| GET `/api/projects/slug/:slug` | public, published only | Vercel fn |
| GET `/api/projects/:id` | JWT | Vercel fn |
| POST `/api/projects/:id/view` | public | Vercel fn |
| POST/PUT/DELETE `/api/projects`, `/api/projects/:id` | JWT | Vercel fn |
| GET `/api/blogs`, `/api/blogs/slug/:slug` | public, published only | Vercel fn |
| POST `/api/blogs/:id/view` | public | Vercel fn |
| GET `/api/admin/blogs`, POST/PUT/DELETE | JWT | Vercel fn |
| GET `/api/cv` | public, active metadata only | Vercel fn |
| GET `/api/cv/download` | public, active PDF | Storage signed URL (Phase 4) |
| GET `/api/admin/cvs`, POST, PUT `:id/active`, DELETE | JWT | Vercel fn |
| POST `/api/admin/images` | JWT, 5MB image | Storage (Phase 4) |
| GET `/api/images/:id` | public, 1y immutable cache | Storage redirect (Phase 4) |
| POST `/api/contact` | public, no-op | retained (no-op) |
| GET `/sitemap.xml` `/api/sitemap.xml` | public | already a Vercel fn (`api/sitemap.js`) |
...
- **Server-side client**: single abstraction `frontend/api/_lib/supabase.js` (service-role,
  never bundled to the browser). Shared helpers to be excluded from Vercel function builds
  via `vercel.json` `functions.excludeFiles`.
...
Vercel functions under `frontend/api/` mirroring the Express route table above, using the
`frontend/api/_lib` client. Caching headers on read-heavy public endpoints. See `Prompt.md` §Phase 3.
...
Legacy (phases 1–5): `JWT_SECRET` (custom JWT until Phase 5 replaces it), `MONGODB_URI`
